...
public class MarioOptions {
	...
	private static Logger log = LoggerFactory.getLogger(MarioOptions.class);
...
//		SYSTEM_TOOLS_CONFIGURATOR("tc", false, "Whether to enable tools configurator (???)."),
	...
	public boolean getBool(BoolOption option) {
		if (bools.containsKey(option)) {
			...
			log.debug("[MarioOptions] " + option.name() + "[-" + option.param + "] specified as: " + value);
			return value;
		}
		
		log.debug("[MarioOptions] " + option.name() + "[-" + option.param + "] default value used: " + option.defaultValue);
	...
	public void setBool(BoolOption option, String strValue) {
		...
		log.debug("[MarioOptions] " + option.name() + "[-" + option.param + "] set as: " + value);
	}
	...
	public void setInt(IntOption option, String strValue) {
		...
		try {
			...
			log.error("[MarioOptions] ~ INVALID VALUE, CANNOT SET " + option.name() + "[-" + option.param + "] AS: " + strValue);
			return;
		}
	...
	public void setOption(String argument, String value) {
		...
		if (option == null) {
			log.error("[MarioOptions] IGNORING UNRECOGNIZED PARAMETER [-" + argument + "] -> " + value);
		} else {
	...
	public void setOption(Object option, String value) {
		...
		if (option instanceof StringOption) {
			...
			log.error("[MarioOptions] CANNOT SET " + option + " AS: " + value);
		}
	}
	...
	public static void read(String... args) {
		...
		log.debug("[MarioOptions] Parsing " + args.length + " arguments ~ " + (args.length / 2) + " parameters:");
		for (String arg : args) {
		...
		log.debug("[MarioOptions] Parameters parsed.");
	}
